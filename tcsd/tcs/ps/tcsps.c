...
int permission_ps_fd = -1;
MUTEX_DECLARE(permission_disk_cache_lock);
static struct flock fl2;
...
int writePermissiontoFile(struct app_permission_disk_cache *node, UINT16 permissionsize, BYTE *permission){
	...
	if ((fd = get_permission_file()) < 0)
		return TCSERR(TSS_E_INTERNAL_ERROR);
	rc =lseek(fd, node->offset, SEEK_SET);
	if (rc == ((off_t) - 1)) {
		LogError("lseek: %s", strerror(errno));
		put_permission_file(fd);
		return TCSERR(TSS_E_INTERNAL_ERROR);
	}
	lseek(fd, sizeof(TSS_UUID)+sizeof(UINT32)+sizeof(UINT16), SEEK_CUR);
	if ((rc = write_data(fd, &permissionsize, sizeof(permissionsize)))) {
			put_permission_file(fd);
			LogError("%s", __FUNCTION__);
			return rc;
	}
	lseek(fd, sizeof(UINT32), SEEK_CUR);/*vendor_data_size*/
	if ((rc = write_data(fd, permission, permissionsize))) {
			put_permission_file(fd);
			LogError("%s", __FUNCTION__);
			return rc;
	}
...
int addPermissiontoFile(struct app_permission_disk_cache *node, UINT32 UID, UINT16 permissionsize, BYTE *permission){
	...
	if ((fd = get_permission_file()) < 0)
		return TCSERR(TSS_E_INTERNAL_ERROR);
	permission_cache_struct.appcounter++;
	rc =lseek(fd, APPCOUNTER_OFFSET, SEEK_SET);
	if ((rc = write_data(fd, (void *)&(permission_cache_struct.appcounter), sizeof(UINT32)))) {
		LogError("%s", __FUNCTION__);
		put_permission_file(fd);
		return TCSERR(TSS_E_INTERNAL_ERROR);
	}
	...
	/* write UUID */
	if ((rc = write_data(fd, (void *)&EUK_UUID, sizeof(TSS_UUID)))) {
		LogError("%s", __FUNCTION__);
		put_permission_file(fd);
		return TCSERR(TSS_E_INTERNAL_ERROR);
	}
	...
	/* write  UID */
	if ((rc = write_data(fd, (void *)&node->uid, sizeof(UINT32)))) {
		LogError("%s", __FUNCTION__);
		put_permission_file(fd);
		return TCSERR(TSS_E_INTERNAL_ERROR);
	}
	UINT32 data = 0;
	char dat[TCSD_MAX_NUM] ={0};
	/* app name data size */
	if ((rc = write_data(fd, &data, sizeof(UINT16)))) {
		LogError("%s", __FUNCTION__);
		put_permission_file(fd);
		return TCSERR(TSS_E_INTERNAL_ERROR);
	}
	/* permission size */
	if ((rc = write_data(fd, &node->permission_size, sizeof(UINT16)))) {
		LogError("%s", __FUNCTION__);
		put_permission_file(fd);
		return TCSERR(TSS_E_INTERNAL_ERROR);
	}
	/*fill 0 to vendor data size*/
	if ((rc = write_data(fd, &data, sizeof(UINT32)))) {
		LogError("%s", __FUNCTION__);
		put_permission_file(fd);
		return TCSERR(TSS_E_INTERNAL_ERROR);
	}
	...
	/*fill 0 to rest addr*/
	if ((rc = write_data(fd, dat, TCSD_MAX_NUM-permissionsize))) {
		LogError("%s", __FUNCTION__);
		put_permission_file(fd);
		return TCSERR(TSS_E_INTERNAL_ERROR);
	}
...
get_permission_file()
{
	...
	int appnum = 0;
	/* check the global file handle first.  If it exists, lock it and return */
	if (permission_ps_fd != -1) {
		 rc = 0;
		fl2.l_type = F_WRLCK;
		if ((rc = fcntl(permission_ps_fd, F_SETLKW, &fl2))) {
			LogError("failed to get system PS lock: %s", strerror(errno));
			return -1;
		}

		return permission_ps_fd;
	}

	/* open and lock the file */
	permission_ps_fd = open(tcsd_options.permission_ps_file, O_CREAT|O_RDWR, 0600);
	if (permission_ps_fd < 0) {
		LogError("system PS: open() of %s failed: %s",
				tcsd_options.permission_ps_file, strerror(errno));
		return -1;
	}
	if ((rc = write_data(permission_ps_fd, &initdata, APPCOUNTER_OFFSET))) {
			LogError("%s", __FUNCTION__);
			return rc;
	}
	if ((rc = write_data(permission_ps_fd, &appnum, sizeof(int)))) {
			LogError("%s", __FUNCTION__);
			return rc;
	}
	fl2.l_type = F_WRLCK;
...
int
put_permission_file(int fd)
{
	int rc = 0;
	/* release the file lock */
	
	fl2.l_type = F_UNLCK;
	if ((rc = fcntl(fd, F_SETLKW, &fl2))) {
		LogError("failed to unlock system PS file: %s",
			strerror(errno));
		return -1;
	}
...
void print_file_lock(){
	if(fl2.l_type == F_WRLCK){
		printf("writeNewHMACtoCache,l_type:F_WRLCK\n");
	}
	else
	{
		printf("writeNewHMACtoCache,l_type:F_UNLCK\n");
	}
}
