...
TSS_RESULT
TCS_SetCredential_Internal(TCS_CONTEXT_HANDLE hContext,		/* in  */
			   UINT32 ulCredentialType,		/* in */
			   UINT32 ulCredentialAccessMode,	/* in */
			   ...
			   BYTE * rgbCredentialData)		/* in */
{
	...
	result = set_credential(ulCredentialType, ulCredentialSize, rgbCredentialData);
	if ( result ) {
		printf("set_credential failed. error: %d\n", result);
		return TCSERR(TSS_E_FAIL);
	}
