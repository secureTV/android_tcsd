...
/*add by xlyu*/
/* divide function  table to 4 levels */
typedef struct tcsdFunctionTable {
	UINT32 order;
	const char *name;
} FunctionTable;
FunctionTable nouse_permission_list[]={/*list redundancy functions in smart TV scene*/
	{34,"DirWriteAuth"},/*no use in tpm 1.2*/
	{35,"DirRead"}, /* 35 */
	{45,"StirRandom"}, /* 45 *//*if use this function, make sure added entropy data is random,else will make  less secure when getrandom,add by xlyu*/
	{51,"DisablePubekRead"},
	{57,"OwnerSetDisable"},
	{58,"OwnerClear"},
	{59,"DisableOwnerClear"},
	{60,"ForceClear"}, /* 60 */
	{61,"DisableForceClear"},
	{62,"PhysicalDisable"},
	{63,"PhysicalEnable"},
	{64,"PhysicalSetDeactivated"},
	{65,"SetTempDeactivated"}, /* 65 */
	{66,"PhysicalPresence"},
	{110,"CreateRevocableEndorsementKeyPair"}, /* 110 */
	{111,"RevokeEndorsementKeyPair"},
};
FunctionTable root_permission_list[]={
	{17,"LogPcrEvent"},
	{18,"GetPcrEvent"},
	{19,"GetPcrEventsByPcr"},
	{20,"GetPcrEventLog"}, /* 20 */
	{21,"SetOwnerInstall"},
	{26,"ChangeAuthOwner"},
	{31,"Extend"},
	{48,"GetCapabilityOwner"},
	{49,"CreateEndorsementKeyPair"},
	{71,"KillMaintenanceFeature"},
	{72,"LoadManuMaintPub"},
	{73,"ReadManuMaintPub"},
	{76,"SetCapability"},
	{77,"ResetLockValue"},
	...
	{81,"IncrementCounter"},
	{82,"ReleaseCounter"},
	{83,"ReleaseCounterOwner"},
	{84,"ReadCurrentTicks"},
	{85,"TicksStampBlob"}, /* 85 */
	{95,"SetOrdinalAuditStatus"}, /* 95 */
	{96,"GetAuditDigest"},
	{97,"GetAuditDigestSigned"},
	{102,"SetTempDeactivated2"},
	{103,"Delegate_Manage"},
	{104,"Delegate_CreateKeyDelegation"},
	{105,"Delegate_CreateOwnerDelegation"}, /* 105 */
	{106,"Delegate_LoadOwnerDelegation"},
	{107,"Delegate_ReadTable"},
	{108,"Delegate_UpdateVerificationCount"},
	{109,"Delegate_VerifyDelegation"}

};
FunctionTable system_permission_list[] = {
	{5,"RegisterKey"}, /* 5 */
	{6,"UnregisterKey"},
	{7,"EnumRegisteredKeys"},
	{9,"GetRegisteredKeyBlob"},
	{10,"GetRegisteredKeyByPublicInfo"}, /* 10 */
	{11,"LoadKeyByBlob"},
	{13,"EvictKey"},
	{14,"CreateWrapKey"},
	{16,"MakeIdentity"},
	...
	{25,"ChangeAuth"}, /* 25 */
	{29,"TerminateHandle"},
	{30,"ActivateIdentity"}, /* 30 */
	{32,"PcrRead"},
	{33,"Quote"},
	{36,"Seal"},
	{37,"UnSeal"},
	{39,"CreateMigrationBlob"},
	{40,"ConvertMigrationBlob"}, /* 40 */
	{41,"AuthorizeMigrationKey"},
	{42,"CertifyKey"},
	{50,"ReadPubek"}, /* 50 */
	{52,"OwnerReadPubek"},
	{53,"SelfTestFull"},
	{54,"CertifySelfTest"},
	{56,"GetTestResult"},
	{69,"CreateMaintenanceArchive"},
	{70,"LoadMaintenanceArchive"}, /* 70 */
	{74,"DaaJoin"},
	{75,"DaaSign"}, /* 75 */
	{79,"ReadCounter"},
	{87,"NVDefineOrReleaseSpace"},
	{88,"NVWriteValue"},
	{89,"NVWriteValueAuth"},
	{90,"NVReadValue"}, /* 90 */
	{91,"NVReadValueAuth"},
	{92,"EstablishTransport"},
	{93,"ExecuteTransport"},
	{94,"ReleaseTransportSigned"},
	{98,"Sealx"}, 
	{99,"SetOperatorAuth"},
	{100,"OwnerReadInternalPub"}, /* 100 */
	{101,"EnumRegisteredKeys2"},
	{113,"Quote2"},
	{114,"CMK_SetRestrictions"},
	{115,"CMK_ApproveMA"}, /* 115 */
	{116,"CMK_CreateKey"},
	{117,"CMK_CreateTicket"},
	{118,"CMK_CreateBlob"},
	{119,"CMK_ConvertMigration"},
	{120,"FlushSpecific"}, /* 120 */
	{121, "KeyControlOwner"},
	{124,"GetPermissionList"},
	{125,"SetPermissionList"},/*125*/
	{126,"CheckStatus"},
	
};
FunctionTable shared_permission_list[] = {
	{0,"Error"},   /* 0 */
	{1,"OpenContext"},
	{2,"CloseContext"},
	{3,"Error"},
...
DispatchTable tcs_func_table[TCSD_MAX_NUM_ORDS] = {
	...
	{tcs_wrap_SetCredential,"SetCredential"},
	{tcs_wrap_AC_GetPermissionList,"GetPermissionList"},
	{tcs_wrap_AC_SetPermissionList,"SetPermissionList"},/*125*/
	{tcs_wrap_AC_CheckStatus,"CheckStatus"},
#endif
};
...
/*different  uid  can access different functions,has different permissions  */
int tcsd_access_control(struct tcsd_thread_data *data)
{
	if(data == NULL){
		LogError("Illegal TCSD");
		return 1;
	}
	printf("function:%s,line:%d\n",__FUNCTION__,__LINE__);
	UINT32  ordinal =data->comm.hdr.u.ordinal;
	int  i=0;
	for(i=0;i<sizeof(nouse_permission_list)/sizeof(FunctionTable);i++)
	{
		printf("function:%s,line:%d\n",__FUNCTION__,__LINE__);
		if(ordinal ==nouse_permission_list[i].order){
			LogError("Illegal TCSD ordinal");
			return 1;
		}
	}
	if(data->callinguid != 0/*root*/){
		for(i=0;i<sizeof(root_permission_list)/sizeof(FunctionTable);i++)
		{
			printf("function:%s,line:%d\n",__FUNCTION__,__LINE__);
			if(ordinal ==root_permission_list[i].order){
				LogError("Illegal uid  use  TCSD ordinal");
				return 1;
			}
		}
	}

	if(data->callinguid >= 10000)/*system and device*/{
		for(i=0;i<sizeof(system_permission_list)/sizeof(FunctionTable);i++)
		{
			printf("function:%s,line:%d\n",__FUNCTION__,__LINE__);
			if(ordinal ==system_permission_list[i].order){
				LogError("Illegal appuid  use TCSD ordinal");
				return 1;
			}
		}
		if(!checkAppPermission(data->callinguid, data->comm.hdr.u.ordinal)){
			LogError("no auth appuid  use TCSD ordinal");
			return 1;
		}	
	}
	return 0;
...
{
#ifdef DEBUG
	printf("data addr:%p,data->comm addr:%p, data->comm.hdr addr :%p, package size addr :%p, num_parms addr:%p\n",data, &(data->comm), &(data->comm.hdr), &(data->comm.hdr.packet_size), &(data->comm.hdr.num_parms));
	printf("\ncomm size:%d, hdr size:%d\n", sizeof(data->comm),  sizeof(data->comm.hdr));
		/* make sure the all the data is present */
	printf("%s, data->comm.hdr.num_parms:%d, addr?:%p\n", __FUNCTION__, data->comm.hdr.num_parms, &(data->comm.hdr.num_parms));
#endif
