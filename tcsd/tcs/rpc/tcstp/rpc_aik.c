...
tcs_wrap_GetCredential(struct tcsd_thread_data *data)
{
	...
	LogDebugFn("thread %ld context %x", THREAD_ID, hContext);
	if((data->callinguid >10000)&&((CredType==TSS_TCS_CREDENTIAL_EKCERT)||(CredType==TSSI_CREDENTIAL_INDEX_AIKCert))){
		initData(&data->comm, 0);
	...
	result = TCS_GetCredential_Internal(hContext, CredType, CredAccessMode, 
					    &CredSize, &CredData);
	LogDebugFn("CredType %0x, CredAccessMode %0x, CredSize %0x", CredType, CredAccessMode, CredSize);
	if (result == TSS_SUCCESS) {
		if ( CredSize ) {
			...
			setData(TCSD_PACKET_TYPE_UINT32, 0, &CredSize, 0, &data->comm);
		}
	} else
		initData(&data->comm, 0);
...
TSS_RESULT
tcs_wrap_SetCredential(struct tcsd_thread_data *data)
{
	TCS_CONTEXT_HANDLE hContext;
	UINT32 CredType;
	UINT32 CredAccessMode; 
	...
	if (getData(TCSD_PACKET_TYPE_UINT32, 3, &CredSize, 0, &data->comm))
		return TCSERR(TSS_E_INTERNAL_ERROR);

	if (CredSize>0) {
		CredData = (BYTE*)malloc(CredSize * sizeof(BYTE));		
		if (CredData == NULL)
			return TCSERR(TSS_E_OUTOFMEMORY);
		if (getData(TCSD_PACKET_TYPE_PBYTE, 4, CredData, CredSize, &data->comm)) {
			free(CredData);
			return TCSERR(TSS_E_INTERNAL_ERROR);
		}
	}
	//**the third app must only shared with SetCredential in type  0x0008XXXX,add by xlyu
	if((data->callinguid>10000)&&((CredType&0x00080000)!=0x00080000)){
		printf("*******no auth to third app********\n\n");
		goto end;
	}
	LogDebugFn("thread %ld context 0x%0x \n\n", THREAD_ID, hContext);
	result = TCS_SetCredential_Internal(hContext, CredType, CredAccessMode,
						CredSize, CredData);
	if(result == TSS_SUCCESS){
		if(CredType == TSSI_CREDENTIAL_INDEX_AIKCert){
			writeSignaltoCache(AIKSIGNAL);
		}
		if(CredType == TSSI_CREDENTIAL_INDEX_EUCert){
			writeSignaltoCache(EUSIGNAL);
		}
	}
end:
	free(CredData);
