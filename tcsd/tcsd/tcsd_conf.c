...
config_set_defaults(struct tcsd_config *conf)
{
	...
	if (conf->unset & TCSD_OPTION_PERMISSION_PSFILE)
		conf->permission_ps_file = strdup(TCSD_DEFAULT_PERMISSION_PS_FILE);
#endif
}
...
#endif
	return result;
#else/*ifdef native service ,no need use config file , else will cause root user can do everything via config file*/
	config_set_defaults(conf);
	return TSS_SUCCESS;
#endif
}
