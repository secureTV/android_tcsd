...
#define TCSD_MAX_NUM 127
extern TSS_UUID SRK_UUID;
extern TSS_UUID EUK_UUID;
#define HMAC_LEN 32 

struct app_permission_disk_cache
{
	 UINT32 offset;
	 TSS_UUID uuid;
	 UINT32 uid;
	 UINT16 appname_size;
        UINT16 permission_size;
	 UINT32 vendor_data_size;
	 BYTE permission[TCSD_MAX_NUM];
        struct app_permission_disk_cache *next;
};

struct permission_disk_cache
{
	 TSS_BOOL  isOwnerShip;
	 TSS_BOOL  isAIKSet;
	 TSS_BOOL  isEUSet;
	 UINT32 appcounter;
	 BYTE digest[HMAC_LEN];
	 struct app_permission_disk_cache *head;
};
